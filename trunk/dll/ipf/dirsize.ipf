...
numbers, as in&colon.
:lines.
  ROOTDIR  4096KiB + 8192KiB = 12288KiB (8.24% of drive)
        ADIR 8192KiB + 0 = 8192KiB  (66.67%)
:elines.
:p.
...
(addition performed in kilobytes). The percentage displayed for the root
directory or the highest level directory selected is the percentage of space
that directory tree contains relative to the total drive space. 
The percentages displayed for the other directories are
the percentage of the used space the directory tree contains, in relation 
to the space used by the entire displayed tree. A graph appears below the line displaying a 
...
but they are empty). Gray text indicates that the directory contains
no files but may or may not contain subdirectories.
:p.
The :hp1.Expand:ehp1. and :hp1.Collapse:ehp1. buttons can take a noticeable amount
of time to process
...
The 0 figure denotes a directory with a truly 0 byte count.
:p.
