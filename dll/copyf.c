...
BOOL AdjustWildcardName (CHAR *oldname,CHAR *newname)
{
  ...
  if(strchr(newname,'*') || strchr(newname,'?')) {
    ...
    CHAR srce[CCHMAXPATHCOMP],dest[CCHMAXPATHCOMP],result[CCHMAXPATHCOMP],*p;
      ...
      *dir = 0;
      if(olddisk == newdisk) {      /* same drive */
        ...
        if(ret == ERROR_INVALID_NAME ||
          ...
          if(TruncName(fullnewname,
                       shortname)) {  /* make 8.3 filename */
            DosError(FERR_DISABLEHARDERR);
            ret = DosCopy(oldname,
                          shortname,
                          DCPY_EXISTING);
            if(!ret) {  /* success -- write longname ea */
              WriteLongName(shortname,
                            fullnewname);
              strcpy(fullnewname,
                     shortname);
        ...
        if(!ret) {                        /* double-check success */
          ...
          if(!rc && st2.cbFile == st.cbFile) {   /* seems to have worked... */
            ...
            if(diskchange) {
              DosError(FERR_ENABLEHARDERR);
              DosQueryPathInfo(oldname,
                               FIL_STANDARD,
                               &dummy,
                               sizeof(FILESTATUS3)); /* force disk change */
            }
...
INT wipeallf (CHAR *string,...)
{
  ...
  if(!DosFindFirst(str,&search_handle,FILE_NORMAL | FILE_DIRECTORY |
     FILE_SYSTEM | FILE_READONLY | FILE_HIDDEN | FILE_ARCHIVED,f,
