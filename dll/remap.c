...
MRESULT EXPENTRY RemapDlgProc(HWND hwnd, ULONG msg, MPARAM mp1, MPARAM mp2)
{
  ...
  case WM_INITDLG:
	...
	WinSendDlgItemMsg(hwnd,
			  MAP_ATTACHTO,
			  LM_INSERTITEM,
			  MPFROM2SHORT(LIT_END, 0), MPFROMP(info->res));
	info = info->next;
      }
    }
    {
  ...
  case WM_CONTROL:
    ...
    case MAP_ATTACHLIST:
      ...
      case LN_ENTER:
	PostMsg(hwnd, WM_COMMAND, MPFROM2SHORT(MAP_ATTACH, 0), MPVOID);
	break;
      }
      break;
    case MAP_DETACHLIST:
      switch (SHORT2FROMMP(mp1)) {
      case LN_ENTER:
	PostMsg(hwnd, WM_COMMAND, MPFROM2SHORT(MAP_DETACH, 0), MPVOID);
	break;
      case LN_SELECT:
	{
	  SHORT x;
	  ...
	  WinSetDlgItemText(hwnd, MAP_ATTACHTO, NullStr);
	  ...
	  if (x >= 0) {
	    *d = 0;
	    WinSendDlgItemMsg(hwnd,
			      MAP_DETACHLIST,
			      LM_QUERYITEMTEXT,
			      MPFROM2SHORT(x, sizeof(d)), MPFROMP(d));
	    if (*d) {
	      ...
	      p2 = (FSQBUFFER2 *) buf;
	      DosError(FERR_DISABLEHARDERR);
	      rc = DosQueryFSAttach(d, 0, FSAIL_QUERYNAME, p2, &len);
	      if (!rc) {
		...
		p += p2->cbFSDName + 1;
		if (p2->cbFSAData)
		  WinSetDlgItemText(hwnd, MAP_ATTACHTO, p);
		else
 ...
 */
	  ...
	  if (!mp2) {
		...
		*d = 0;
		WinSendMsg(hwndList,
			   ...
			   MPFROM2SHORT(x, sizeof(d)), MPFROMP(d));
		if (*d == info->device) {
		  WinSendMsg(hwndList, LM_DELETEITEM, MPFROMSHORT(x), MPVOID);
		  hwndList = WinWindowFromID(hwnd,
					     (info->attach) ?
					     MAP_DETACHLIST : MAP_ATTACHLIST);
		  d[1] = ':';
		  d[2] = 0;
		  WinSendMsg(hwndList,
			     LM_INSERTITEM,
			     MPFROM2SHORT(LIT_SORTASCENDING, 0), MPFROMP(d));
		  break;
		}
	      }
	    }
  ...
  case WM_COMMAND:
    ...
    case MAP_CLEAR:
      free_resources();
      save_resources();
      WinSendDlgItemMsg(hwnd, MAP_ATTACHTO, LM_DELETEALL, MPVOID, MPVOID);
      WinSetDlgItemText(hwnd, MAP_ATTACHTO, NullStr);
      break;
	...
	*s = 0;
	WinQueryDlgItemText(hwnd, MAP_ATTACHTO, sizeof(s), s);
	bstrip(s);
	x = (SHORT) WinSendDlgItemMsg(hwnd,
				      MAP_DETACHLIST,
				      LM_QUERYSELECTION,
	  ...
	  *d = 0;
	  ...
	  if (*d) {
		...
		happ = WinStartApp(hwnd,
				   &pgd,
				   pgd.pszParameters, NULL, SAF_MAXIMIZED);
		if (happ) {
	      ...
	      DosError(FERR_DISABLEHARDERR);
	      rc = DosFSAttach(d, s, d, strlen(d) + 1, FS_DETACH);
	      if (rc) {
		Dos_Error(MB_CANCEL,
			  rc,
			  hwnd,
			  pszSrcFile,
			  __LINE__, GetPString(IDS_DETACHFAILEDTEXT), d, s);
	      }
	      else {
		fRemapped = TRUE;
		WinSendDlgItemMsg(hwnd,
	...
	*s = 0;
	  ...
	  if (x >= 0) {
	    *d = 0;
		...
		Dos_Error(MB_CANCEL,
			  ...
			  hwnd,
			  pszSrcFile,
			  __LINE__, GetPString(IDS_ATTACHFAILEDTEXT), s, d);
	      }
	      else {
		fRemapped = TRUE;
		WinSendDlgItemMsg(hwnd,
	...
	WinSendMsg(hwndHelp,
		   HM_DISPLAY_HELP,
		   MPFROM2SHORT(HELP_REMAP, 0), MPFROMSHORT(HM_RESOURCEID));
      break;
