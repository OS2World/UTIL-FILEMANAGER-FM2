...
MRESULT EXPENTRY IniProc(HWND hwnd, ULONG msg, MPARAM mp1, MPARAM mp2)
{
  ...
  case WM_COMMAND:
	...
	if (inirec) {
	  ...
	  strcpy(inirec->filename1, inidata->ininame);
	  inirec->hwndDlg = hwnd;
	  if (_beginthread(CompareIniThread, NULL, 122880, (PVOID) inirec) ==
	      -1) {
	    Runtime_Error(pszSrcFile, __LINE__,
			  GetPString(IDS_COULDNTSTARTTHREADTEXT));
	    free(inirec);
	...
	if (export_filename(hwnd, filename, TRUE)) {
	  ...
	  if (prfp) {
		...
		if (_beginthread(BackupIniThread, NULL, 122880, (PVOID) prfp)
		    == -1) {
		  Runtime_Error(pszSrcFile, __LINE__,
				GetPString(IDS_COULDNTSTARTTHREADTEXT));
		  free(prfp->pszSysName);
	...
	*inidata->ininame = 0;
    ...
    case INI_DELETEKEY:
      inidata = INSTDATA(hwnd);
      if (inidata && *inidata->applname && *inidata->keyname) {
