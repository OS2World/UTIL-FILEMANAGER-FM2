...
typedef struct {
  ...
  ULONG     affiles,afalloc,afFilesToGet,longest,longestw,
	    topfile,cursored,selected,ulSelectedKB, ulSelectedBytes,afifiles,
...
static VOID SelectMask (HWND hwnd,BOOL deselect)
{
  ...
  if (WinDlgBox(HWND_DESKTOP,
     ...
     (*mask.szMask ||
      mask.attrFile != pAD->mask.attrFile ||
      mask.antiattr != pAD->mask.antiattr)) {
    for (x = 0;x < pAD->afifiles;x++) {
      y = (pAD->invertsort) ? (pAD->afifiles - 1) - x : x;
      ret = FALSE;
      if (mask.pszMasks[1]) {
	...
	ret = TRUE;
      if (ret) {
	if ((!(mask.attrFile & FILE_HIDDEN) && (pAD->afindex[y]->attrFile & FILE_HIDDEN)) ||
	   (!(mask.attrFile & FILE_SYSTEM) && (pAD->afindex[y]->attrFile & FILE_SYSTEM)) ||
	   (!(mask.attrFile & FILE_READONLY) && (pAD->afindex[y]->attrFile & FILE_READONLY)) ||
	   (!(mask.attrFile & FILE_ARCHIVED) && (pAD->afindex[y]->attrFile & FILE_ARCHIVED)))
	  ret = FALSE;
	else if (((mask.antiattr & FILE_HIDDEN) && !(pAD->afindex[y]->attrFile & FILE_HIDDEN)) ||
		((mask.antiattr & FILE_SYSTEM) && !(pAD->afindex[y]->attrFile & FILE_SYSTEM)) ||
		((mask.antiattr & FILE_READONLY) && !(pAD->afindex[y]->attrFile & FILE_READONLY)) ||
		((mask.antiattr & FILE_ARCHIVED) && !(pAD->afindex[y]->attrFile & FILE_ARCHIVED)))
	 ret = FALSE;
      }
      if (ret) {
	if (deselect) {
	  if (pAD->afindex[y]->flags & AF_SELECTED) {
	    pAD->selected--;
	    ul = pAD->afindex[y]->cbFile;
	    pAD->ulSelectedKB -= ul >> 10;
	    pAD->ulSelectedBytes -= ul & 0x3ff;
...
static VOID ReSort (HWND hwnd) {
  ...
  pAD->selected = 0;
  pAD->ulSelectedKB = 0;
  pAD->ulSelectedBytes = 0;
  for (x = 0,y = 0;x < pAD->affiles;x++) {
  ...
  if(!pAD->stopflag && pAD->compare && pAD->afifiles) {
    WinSendMsg(hwnd,UM_RESCAN,MPFROMLONG(1L),MPVOID);
    qsort(pAD->afindex,pAD->afifiles,sizeof(ALLFILES *),pAD->compare);
  }
}
  ...
  for (x = 0;x < pAD->affiles;x++) {
	...
	free(pAD->afhead[y].fullname);
      ...
      memmove(&(pAD->afhead[x]),&(pAD->afhead[y]),
	      (pAD->affiles - y) * sizeof(ALLFILES));
      pAD->affiles -= (y - x);
    }
  } // for x
  if(pAD->affiles != oldaffiles) {
...
MRESULT EXPENTRY SeeAllWndProc (HWND hwnd, ULONG msg, MPARAM mp1, MPARAM mp2) {
  ...
  switch (msg) {
	...
	*(pAD->mask.prompt) = 0;
	...
	switch(SortType) {
	  case IDM_SORTEASIZE:
	    pAD->compare = (PFNSORT)NULL;
	    break;
	  case IDM_SORTNAME:
	    pAD->compare = comparefullnames;
	    break;
	  case IDM_SORTFILENAME:
	    pAD->compare = comparenames;
	    break;
	  case IDM_SORTSIZE:
	    pAD->compare = comparesizes;
	    break;
	  case IDM_SORTLWDATE:
	    pAD->compare = comparedates;
	    break;
	  case IDM_SORTFIRST:
	    pAD->compare = compareexts;
	    break;
	}
	...
	memcpy(pAD->colors,Colors,sizeof(LONG) * COLORS_MAX);
	...
	if(_beginthread(MakeSeeObj,NULL,122880,(PVOID)pAD) != -1) {
	  if(!DosCreateMutexSem(NULL,&pAD->ScanSem,0L,FALSE)) {
	...
	rc = DosRequestMutexSem(pAD->ScanSem,SEM_IMMEDIATE_RETURN);
	if(!rc) {
	  ...
	  DosReleaseMutexSem(pAD->ScanSem);
	...
	if(!*pAD->findpath) {
	  ...
	  if(!DosQCurDisk(&ulDriveNum,&ulDriveMap)) {
	    for(x = 2L;x < 26L && !pAD->stopflag;x++) {
	      if((ulDriveMap & (1L << x)) && pAD->drvsflags[x]) {
		sprintf(&s[strlen(s)],
			"%s%c:",
			(once) ? ", " : " (",
			x + 'A');
...
// fprintf(stderr,"Seeall: UM_SETUP3\n");
      if(pAD) {
	pAD->multiplier = pAD->afifiles / 32767;
	if(pAD->multiplier * 32767 != pAD->afifiles)
	  pAD->multiplier++;
	if(!pAD->multiplier)
	  pAD->multiplier++;
	{
	  ...
	  if(numlines) {
	    ...
	    WinSendMsg(pAD->hvscroll,SBM_SETTHUMBSIZE,
		       MPFROM2SHORT((SHORT)numlines,
				    (SHORT)min(pAD->afifiles,32767)),
		       MPFROM2SHORT(1,pAD->afifiles + 1));
	    ...
	    WinSendMsg(pAD->hvscroll,SBM_SETSCROLLBAR,
		       ...
		       MPFROM2SHORT(1,
				    (SHORT)(pAD->afifiles / pAD->multiplier) -
					    (numlines - 1)));
	    if(pAD->afifiles - pAD->topfile < numlines) {
	      pAD->topfile = ((pAD->afifiles - pAD->topfile) - numlines);
	      WinInvalidateRect(hwnd,NULL,FALSE);
	    }
	  }
	}
...
// fprintf(stderr,"Seeall: UM_SETUP4\n");
    ...
    case UM_RESCAN:
      ...
      if(pAD && !pAD->stopflag) {
	if(DosRequestMutexSem(pAD->ScanSem,SEM_IMMEDIATE_RETURN)) {
	  ...
	  if(mp1) {
	    ...
	    if(pAD->afifiles) {
	      *tm = 0;
	      commafmt(tm,sizeof(tm),pAD->afifiles);
	      strcat(s,tm);
	    }
	  }
	  else {
	  ...
	  else if(pAD->afifiles) {
	    y = (pAD->invertsort) ? (pAD->afifiles - 1) - (pAD->cursored - 1) :
				   pAD->cursored - 1;
	    *tm = *ts = *tb = 0;
	    commafmt(tm,sizeof(tm),pAD->afifiles);
	    ...
	    if ((long)ul > 0) {
	      ...
	      ul = -ul;
	      pAD->ulSelectedKB -= (ul >> 10) + 1;
	      pAD->ulSelectedBytes = 1024 - (ul & 0x3ff);
	    }

	    commafmt(tb,sizeof(tb),pAD->ulSelectedKB);
	    sprintf(s,
		    " %s %s%s%s  %s %s (%sk)  %s %s",
		    tm,
...
// fprintf(stderr,"Seeall: UM_SETUP\n");
	...
	register ULONG  x;
	ULONG           numlines,y,wascursored = pAD->cursored,
			thistime,len;
	BOOL            found = FALSE;
	RECTL           rcl;
	    ...
	    *pAD->comnam = 0;
		...
		if(pAD->cursored > 1) {
		  ...
		  if(pAD->cursored < pAD->topfile) {
		    ...
		    PaintLine(hwnd,pAD->hps,pAD->cursored - 1,pAD->topfile,&rcl);
		    PaintLine(hwnd,pAD->hps,pAD->cursored,pAD->topfile,&rcl);
		  }
		  PostMsg(hwnd,UM_RESCAN,MPVOID,MPVOID);
		}
		break;
	      case VK_DOWN:
		if(pAD->cursored < pAD->afifiles && pAD->cursored < pAD->topfile + numlines) {
		  if(shiftstate & KC_SHIFT)
		    WinSendMsg(hwnd,WM_BUTTON1CLICK,
			       MPFROM2SHORT(pAD->fattrs.lAveCharWidth + 2,
					    ((rcl.yTop - (pAD->lMaxHeight *
		  ...
		  if(pAD->cursored >= pAD->topfile + numlines) {
		    PaintLine(hwnd,pAD->hps,pAD->cursored - 2,pAD->topfile,&rcl);
		    WinSendMsg(hwnd,WM_VSCROLL,MPFROM2SHORT(FID_VERTSCROLL,0),
		...
		if((shiftstate & KC_CTRL) ||
		  ...
		  pAD->cursored = pAD->afifiles;
		  pAD->topfile = (pAD->afifiles + 1) - numlines;
		  if(pAD->topfile > pAD->afifiles)
		    pAD->topfile = 1;
		  WinInvalidateRect(hwnd,NULL,FALSE);
		}
		else {
		  pAD->cursored = (pAD->topfile - 1) + numlines;
		  if(pAD->cursored > pAD->afifiles)
		    pAD->cursored = pAD->afifiles;
		  PaintLine(hwnd,pAD->hps,pAD->cursored - 1,pAD->topfile,&rcl);
...
KbdRetry:
		len = strlen(pAD->comnam);
		if(len >= CCHMAXPATH - 1) {
		  *pAD->comnam = 0;
		  len = 0;
		}
		pAD->comnam[len] = toupper(SHORT1FROMMP(mp2));
		pAD->comnam[len + 1] = 0;
		for(x = pAD->cursored - (len > 0);x < pAD->afifiles;x++) {
		  y = (pAD->invertsort) ? (pAD->afifiles - 1) - x : x;
		  if(pAD->fullnames) {
		    if(!strnicmp(pAD->afindex[y]->fullname,pAD->comnam,
				 len + 1)) {
		      found = TRUE;
		      break;
		    }
		    ...
		    if(!strnicmp(pAD->afindex[y]->filename,pAD->comnam,
				 len + 1)) {
		      found = TRUE;
		      break;
		    }
		...
		if(found) {
		  if(x + 1 != pAD->cursored) {
			...
			pAD->topfile = 1;
		      else if(pAD->cursored > (pAD->afifiles + 1) - (numlines / 2))
			pAD->topfile = (pAD->afifiles + 1) - numlines;
		      else
    ...
    case WM_BUTTON1MOTIONSTART:
      if(pAD && !pAD->stopflag &&
	 !DosRequestMutexSem(pAD->ScanSem,SEM_IMMEDIATE_RETURN)) {
    ...
    case WM_CONTEXTMENU:
      if(pAD) {
	if(!pAD->hwndPopup) {
	  pAD->hwndPopup = WinLoadMenu(HWND_DESKTOP,FM3ModHandle,SEEALL_POPUP);
	  if(pAD->hwndPopup) {
	    ...
	    SetConditionalCascade(pAD->hwndPopup,IDM_OPENSUBMENU,
				  IDM_OPENDEFAULT);
	    SetConditionalCascade(pAD->hwndMenu,IDM_OBJECTSUBMENU,
				  IDM_SHADOW);
	    if(fWorkPlace) {
	...
	if(pAD->hwndPopup) {
	  ...
	  WinEnableMenuItem(pAD->hwndPopup,IDM_OPENSUBMENU,(rc == 0 &&
							   pAD->selected != 0));
	  WinEnableMenuItem(pAD->hwndPopup,IDM_OBJECTSUBMENU,(rc == 0 &&
							     pAD->selected != 0));
	  WinEnableMenuItem(pAD->hwndPopup,IDM_DELETESUBMENU,(rc == 0 &&
							     pAD->selected != 0));
	  WinEnableMenuItem(pAD->hwndPopup,IDM_INFO,(rc == 0 &&
						    pAD->selected != 0));
	  WinEnableMenuItem(pAD->hwndPopup,IDM_ATTRS,(rc == 0 &&
						     pAD->selected != 0));
	  WinEnableMenuItem(pAD->hwndPopup,IDM_COLLECT,(rc == 0 &&
						       pAD->selected != 0));
	  WinEnableMenuItem(pAD->hwndPopup,IDM_SAVETOCLIP,(rc == 0 &&
							  pAD->selected != 0));
	  WinEnableMenuItem(pAD->hwndPopup,IDM_APPENDTOCLIP,(rc == 0 &&
							    pAD->selected != 0));
	  WinEnableMenuItem(pAD->hwndPopup,IDM_SAVETOLIST,(rc == 0 &&
							  pAD->selected != 0));
	  WinEnableMenuItem(pAD->hwndPopup,IDM_REMOVE,(rc == 0 &&
						      pAD->selected != 0));
	  WinEnableMenuItem(pAD->hwndPopup,IDM_HIDEALL,(rc == 0 &&
						      pAD->selected != 0));
	  WinEnableMenuItem(pAD->hwndPopup,IDM_SELECTALL,(rc == 0 &&
							 pAD->afifiles != 0));
	  WinEnableMenuItem(pAD->hwndPopup,IDM_SELECTMASK,(rc == 0 &&
							  pAD->afifiles != 0));
	  WinEnableMenuItem(pAD->hwndPopup,IDM_DESELECTALL,(rc == 0 &&
							   pAD->afifiles != 0));
	  WinEnableMenuItem(pAD->hwndPopup,IDM_DESELECTMASK,(rc == 0 &&
							    pAD->afifiles != 0));
	  WinEnableMenuItem(pAD->hwndPopup,IDM_INVERT,(rc == 0 &&
						      pAD->afifiles != 0));
	  WinEnableMenuItem(pAD->hwndPopup,IDM_FILTER,(rc == 0 &&
						      pAD->affiles != 0));
	  if(!rc)
	    DosReleaseMutexSem(pAD->ScanSem);
...
// fprintf(stderr,"Seeall: WM_PAINT\n");
	...
	if(!pAD->stopflag &&
	  ...
	  WinQueryWindowRect(hwnd,&Rectl);
	  numlines = NumLines(&Rectl,pAD);
	  if(pAD->afifiles && numlines) {
	    if(pAD->topfile > (pAD->afifiles + 1) - numlines)
	      pAD->topfile = (pAD->afifiles + 1) - numlines;
	    if(pAD->topfile > pAD->afifiles)
	      pAD->topfile = 1;
	    if(!pAD->topfile)
	  ...
	  if(numlines && pAD->afifiles) {
	    GpiSetBackMix(hpsp,BM_OVERPAINT);
	    wascursored = TRUE;
	    for(x = pAD->topfile - 1;x < pAD->afifiles;x++) {
		...
		break;
	      if(pAD->afindex[y]->flags & AF_SELECTED) {
		if(!inverted) {
		  GpiSetColor(pAD->hps,standardcolors[Colors[COLORS_SELECTEDNORMALFORE]]);
		  GpiSetBackColor(pAD->hps,(wascursored) ?
				  standardcolors[Colors[COLORS_CURSOREDSELECTEDBACK]] :
				  standardcolors[Colors[COLORS_SELECTEDBACK]]);
		  inverted = TRUE;
		}
		...
		if(pAD->afindex[y]->attrFile & (FILE_SYSTEM | FILE_HIDDEN)) {
		  ...
		  GpiSetColor(pAD->hps,standardcolors[Colors[COLORS_NORMALFORE]]);
		GpiSetBackColor(pAD->hps,(wascursored) ?
				standardcolors[Colors[COLORS_CURSOREDNORMALBACK]] :
				standardcolors[Colors[COLORS_NORMALBACK]]);
		inverted = FALSE;
		...
		GpiSetBackColor(pAD->hps,
	      ...
	      len = sprintf(szBuff,
			    ...
			    (pAD->fullnames) ? pAD->longestw : pAD->longest,
			    (pAD->fullnames) ? pAD->afindex[y]->fullname :
					      pAD->afindex[y]->filename,
			    pAD->afindex[y]->cbFile,
			    "-A"[((pAD->afindex[y]->attrFile & FILE_ARCHIVED) != 0)],
			    "-R"[((pAD->afindex[y]->attrFile & FILE_READONLY) != 0)],
			    "-H"[((pAD->afindex[y]->attrFile & FILE_HIDDEN) != 0)],
			    "-S"[((pAD->afindex[y]->attrFile & FILE_SYSTEM) != 0)],
			    "-D"[((pAD->afindex[y]->attrFile & FILE_DIRECTORY) != 0)],
			    pAD->afindex[y]->date.year + 1980,
			    pAD->afindex[y]->date.month,
			    pAD->afindex[y]->date.day,
...
// fprintf(stderr,"Seeall: WM_VSCROLL\n");
	...
	if(pAD->afifiles) {
	  ...
	  if(numlines) {
		...
		break;
	      case SB_PAGEDOWN:
		if(pAD->topfile <= pAD->afifiles - numlines) {
		  pAD->topfile += numlines;
		  if(pAD->topfile > (pAD->afifiles + 1) - numlines)
		    pAD->topfile = (pAD->afifiles + 1) - numlines;
		  if(pAD->cursored < pAD->topfile)
		...
		if(pAD->topfile <= pAD->afifiles - numlines) {
		  ...
		  PaintLine(hwnd,pAD->hps,(pAD->topfile + numlines) - 2,
			    pAD->topfile,&Rectl);
		  if(pAD->cursored != pAD->topfile + numlines)
		    PaintLine(hwnd,pAD->hps,pAD->cursored - 1,pAD->topfile,&Rectl);
		  ...
		  WinSendMsg(pAD->hvscroll,SBM_SETSCROLLBAR,
			     MPFROMSHORT((SHORT)(pAD->topfile /
						 pAD->multiplier)),
			     MPFROM2SHORT(1,(SHORT)(pAD->afifiles /
						    pAD->multiplier) -
						    (numlines - 1)));
		}
		break;
		...
		if(pAD->topfile > 1) {
		  ...
		  iRectl.yTop -= ((numlines * pAD->lMaxHeight) +
				  pAD->lMaxDescender);
		  WinFillRect(pAD->hps,&iRectl,
			      standardcolors[pAD->colors[COLORS_NORMALBACK]]);
		  PaintLine(hwnd,pAD->hps,pAD->topfile - 1,pAD->topfile,&Rectl);
		  if(pAD->cursored != pAD->topfile)
		    PaintLine(hwnd,pAD->hps,pAD->cursored - 1,pAD->topfile,&Rectl);
		  if(pAD->cursored != wascursored &&
		     wascursored >= pAD->topfile &&
	...
	case IDM_DUPES:
	  if(!pAD->stopflag &&
	     !DosRequestMutexSem(pAD->ScanSem,
	...
	case IDM_RESCAN:
	  if(!pAD->stopflag &&
	    ...
	    memcpy(tempflags,pAD->drvsflags,sizeof(tempflags));
	    if(!WinDlgBox(HWND_DESKTOP,hwnd,AFDrvsWndProc,FM3ModHandle,
			  DRVS_FRAME,(PVOID)pAD)) {
	      memcpy(pAD->drvsflags,tempflags,sizeof(tempflags));
	      *pAD->findpath = 0;
	      DosReleaseMutexSem(pAD->ScanSem);
	      break;
	    }
	      ...
	      case IDM_UUDECODE:
		{
		  ...
		  if(list) {
			...
			break;
		      ...
		      case IDM_OPENSETTINGS:
		      case IDM_OPENDEFAULT:
			if(!PostMsg(pAD->hwndObj,UM_MASSACTION,mp1,
				       MPFROMP(list)))
			  FreeList(list);
			break;
		    }
...
// fprintf(stderr,"Seeall: WM_DESTROY\n");
	...
	pAD->stopflag = 1;
	if(pAD->ScanSem) {
	  DosRequestMutexSem(pAD->ScanSem,15000L);
	  DosCloseMutexSem(pAD->ScanSem);
	}
	if(pAD->hwndPopup)
	  WinDestroyWindow(pAD->hwndPopup);
	if(pAD->hwndObj) {
	  if(!PostMsg(pAD->hwndObj,
		      WM_CLOSE,
		      MPVOID,
		      MPVOID))
	    WinSendMsg(pAD->hwndObj,
		       WM_CLOSE,
		       MPVOID,
		       MPVOID);
	}
	if(pAD->hps) {
	  GpiDeleteSetId(pAD->hps,
			 FIXED_FONT_LCID);
	  GpiAssociate(pAD->hps,0);
	  GpiDestroyPS(pAD->hps);
	}
...
HWND StartSeeAll (HWND hwndParent,
		  BOOL standalone,	// called by applet
