      ...
      if(hab2) {
        ...
        if(hmq2) {
          ...
          BOOL         dontask = FALSE,wildcarding = FALSE,
                       overold = FALSE,overnew = FALSE,usedtarget;
                ...
                case IDM_UUDECODE:
                  {
                    ...
                    sprintf(message,
                            GetPString(IDS_UUDECODINGTEXT),
                            wk->li->list[x]);
                    AddNote(message);
                    if(UUD(wk->li->list[x],outname) &&
                       *outname) {
                      sprintf(message,
...
Retry:
                  {
                    ...
                    type = (wk->li->type == IDM_RENAME) ? MOVE :
                           ...
                           (wk->li->type == IDM_WPSCOPY) ? WPSCOPY :
                           COPY;
                    ...
                    if((wk->li->type == IDM_RENAME &&
                      ...
                      if(wildcarding || wk->li->type == IDM_RENAME) {
                        ...
                        if(p && (strchr(p,'*') || strchr(p,'?'))) {
                          ...
                          AdjustWildcardName(wk->li->list[x],
                                             mv.target);
                        }
                        else
                          *wildname = 0;
                      ...
                      isnewer = IsNewer(wk->li->list[x],newname);
                      if(!mv.overwrite) {
                        if(existed && wk->li->type != IDM_RENAME &&
                           dontask) {
                          if(!overold && !overnew)
                            break;
                          if(!overold && !isnewer)
    ...
    if(wk->li && wk->li->list && wk->li->list[0]) {
      ...
      if(hab2) {
          ...
          switch(wk->li->type) {
            ...
            case IDM_DOITYOURSELF:
              ExecOnList(wk->hwndFrame,
                         ...
                         NULL,
                         wk->li->list,
                         GetPString(IDS_DOITYOURSELFTEXT));
              break;
            ...
            case IDM_SETICON:
              if(*wk->li->targetpath) {
                ...
                if(!WinSetFileIcon((PSZ)wk->li->targetpath,
                  ...
                  WinSetFileIcon((PSZ)wk->li->targetpath,
                                 (PICONINFO)&ici);
                }
                Broadcast(hab2,
                          wk->hwndCnr,
		...
		char		*pch;
                ...
                if(*wk->li->targetpath &&
                   IsFile(wk->li->targetpath) > 0) {
                  info = find_type(wk->li->targetpath,NULL);
                  ad.namecanchange = 0;
                }
                else {
                  if(*wk->li->targetpath &&
                     !IsFile(wk->li->targetpath))
                    if(wk->li->targetpath[strlen(wk->li->targetpath) - 1] != '\\')
                      strcat(wk->li->targetpath,"\\");
                  ad.namecanchange = 1;
                }
                ...
                for(x = 0;wk->li->list[x];x++) {
                  ...
                  if(fsa.attrFile & FILE_DIRECTORY) {
                    ...
                    error = (APIRET)wipeallf("%s%s*",
                                             wk->li->list[x],
                                             (*wk->li->list[x] &&
                                              wk->li->list[x][strlen(wk->li->list[x]) - 1] !=
                      '\\') ?
                       "\\" :
                       NullStr);
                    DosError(FERR_DISABLEHARDERR);
