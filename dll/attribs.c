  ...
  switch(msg) {
    case WM_INITDLG:
      ...
      WinSendDlgItemMsg(hwnd,ATR_SECONDS,SPBM_SETTEXTLIMIT,MPFROMSHORT(2L),
                        MPVOID);
      WinSendDlgItemMsg(hwnd,ATR_YEAR,SPBM_OVERRIDESETLIMITS,MPFROMLONG(2200L),
                        MPFROMLONG(1980L));
      WinSendDlgItemMsg(hwnd,ATR_MONTH,SPBM_OVERRIDESETLIMITS,MPFROMLONG(12L),
                        MPFROMLONG(1L));
      ...
      WinSendDlgItemMsg(hwnd,ATR_HOUR,SPBM_OVERRIDESETLIMITS,MPFROMLONG(23L),
                        MPFROMLONG(0L));
      WinSendDlgItemMsg(hwnd,ATR_MINUTES,SPBM_OVERRIDESETLIMITS,MPFROMLONG(59L),
                        MPFROMLONG(0L));
      WinSendDlgItemMsg(hwnd,ATR_SECONDS,SPBM_OVERRIDESETLIMITS,MPFROMLONG(59L),
                        MPFROMLONG(0L));
    ...
    case UM_UNDO:
      {
        ...
        if(li && li->list[0] && (allgrey || mp1) &&
          ...
          WinSendDlgItemMsg(hwnd,ATR_YEAR,SPBM_SETCURRENTVALUE,
                            MPFROMSHORT(fi.fdateLastWrite.year + 1980),
                            MPVOID);
        ...
        PostMsg(hwnd,UM_CONTROL,MPFROM2SHORT(ATR_READONLY,BN_CLICKED),MPVOID);
        ...
        PostMsg(hwnd,UM_CONTROL,MPFROM2SHORT(ATR_ARCHIVED,BN_CLICKED),MPVOID);
      }
      return 0;
    ...
    case UM_SETDIR:
      PaintRecessedWindow(WinWindowFromID(hwnd,ATR_HELP),(HPS)0,FALSE,TRUE);
      return 0;
      ...
      switch(SHORT1FROMMP(mp1)) {
        ...
        case ATR_YEAR:
          ...
          if(SHORT2FROMMP(mp1) == SPBN_SETFOCUS)
            WinSetDlgItemText(hwnd,ATR_HELP,
                              GetPString(IDS_ATTRYEARHELPTEXT));
          break;
        case ATR_MONTH:
          if(SHORT2FROMMP(mp1) == SPBN_KILLFOCUS)
            WinSetDlgItemText(hwnd,ATR_HELP,NullStr);
          if(SHORT2FROMMP(mp1) == SPBN_SETFOCUS)
            WinSetDlgItemText(hwnd,ATR_HELP,
                              GetPString(IDS_ATTRMONTHHELPTEXT));
          break;
        case ATR_DAY:
          if(SHORT2FROMMP(mp1) == SPBN_KILLFOCUS)
            WinSetDlgItemText(hwnd,ATR_HELP,NullStr);
          if(SHORT2FROMMP(mp1) == SPBN_SETFOCUS)
            WinSetDlgItemText(hwnd,ATR_HELP,
                              GetPString(IDS_ATTRDAYHELPTEXT));
          break;
        case ATR_HOUR:
          if(SHORT2FROMMP(mp1) == SPBN_KILLFOCUS)
            WinSetDlgItemText(hwnd,ATR_HELP,NullStr);
          if(SHORT2FROMMP(mp1) == SPBN_SETFOCUS)
            WinSetDlgItemText(hwnd,ATR_HELP,
                              GetPString(IDS_ATTRHOURHELPTEXT));
          break;
        case ATR_MINUTES:
          if(SHORT2FROMMP(mp1) == SPBN_KILLFOCUS)
            WinSetDlgItemText(hwnd,ATR_HELP,NullStr);
          if(SHORT2FROMMP(mp1) == SPBN_SETFOCUS)
            WinSetDlgItemText(hwnd,ATR_HELP,
                              GetPString(IDS_ATTRMINHELPTEXT));
          break;
        case ATR_SECONDS:
          if(SHORT2FROMMP(mp1) == SPBN_KILLFOCUS)
            WinSetDlgItemText(hwnd,ATR_HELP,NullStr);
          if(SHORT2FROMMP(mp1) == SPBN_SETFOCUS)
            WinSetDlgItemText(hwnd,ATR_HELP,
                              GetPString(IDS_ATTRSECHELPTEXT));
          break;
        case ATR_READONLY:
        case ATR_SYSTEM:
        case ATR_HIDDEN:
        ...
        case ATR_ARCHIVED:
          {
            ...
            sprintf(s,"%s %s",
                    ...
                    (check == 1) ?
                     ...
                     (check == 0) ?
                      GetPString(IDS_OFFTEXT) :
                       GetPString(IDS_UNCHANGEDTEXT));
      ...
      switch(SHORT1FROMMP(mp1)) {
        ...
        case DID_OK:
          if(!WinQueryButtonCheckstate(hwnd,ATR_USEDATETIME) &&
            ...
            saymsg(MB_ENTER,
                   hwnd,
                   GetPString(IDS_UHYOTEXT),
                   GetPString(IDS_ATTRADVANTAGETEXT));
            break;
          }
          {
            ULONG       temp = 0L;
            ...
            for(x = 0;li->list[x];x++) {
              ...
              if(WinQueryButtonCheckstate(hwnd,ATR_USEDATETIME)) {
                ...
                WinSendDlgItemMsg(hwnd,ATR_MONTH,SPBM_QUERYVALUE,MPFROMP(&temp),
                                  MPFROM2SHORT(0,SPBQ_DONOTUPDATE));
                ...
                WinSendDlgItemMsg(hwnd,ATR_MINUTES,SPBM_QUERYVALUE,MPFROMP(&temp),
                                  MPFROM2SHORT(0,SPBQ_DONOTUPDATE));
                fi.ftimeLastWrite.minutes = temp;
                WinSendDlgItemMsg(hwnd,ATR_SECONDS,SPBM_QUERYVALUE,MPFROMP(&temp),
                                  MPFROM2SHORT(0,SPBQ_DONOTUPDATE));
